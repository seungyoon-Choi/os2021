...
void
push_ready_list_in_priority_order(const struct list_elem *elem) {
	struct list_elem *e = list_begin(&ready_list);
	if(!list_empty(&ready_list)) {
		struct thread *t1 = list_entry(e, struct thread, elem);
		struct thread *t2 = list_entry(elem, struct thread, elem);

		while((t1->priority >= t2->priority) && (e != list_end(&ready_list))) {
			e = list_next(e);
...
update_priority(void) {
	...
	if(!list_empty(&current_thread->donated_by)) {
		struct list_elem *e = list_begin(&current_thread->donated_by);
		struct thread *highest = list_entry(e, struct thread, donator);
		if(current_thread->priority < highest->priority) {
		       current_thread->priority = highest->priority;	
		}
	}
}
...
remove_lock(struct lock *lock) {
	...
	while(e != list_end(&current_thread->donated_by)) {
...
thread_unblock (struct thread *t) {
	...
	push_ready_list_in_priority_order(&t->elem);
...
thread_yield (void) {
	...
	if (curr != idle_thread)
		...
		push_ready_list_in_priority_order(&curr->elem);
